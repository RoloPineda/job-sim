...
There are three agent types: jobseekers, recruiters, and hiring managers. Together they form the core interaction loop of the simulation: seekers apply to postings, recruiters screen and forward candidates, and hiring managers make final decisions.
...
Operates as the bridge between seekers and hiring managers. Screens incoming applications, forwards promising candidates with a written assessment, sends status updates and rejections to candidates, and nudges unresponsive hiring managers.
...
- `events_created`: Status notifications directed at seekers (rejections, advances).

### Hiring Manager

The final decision-maker on candidates. Reviews forwarded candidates, conducts interviews, provides feedback to recruiters, approves or rejects candidates, and can request changes to postings.

The hiring manager's `feedback_clarity` trait (clear, vague, contradictory) affects how useful its feedback is to the recruiter, which in turn affects screening quality over time.

**Side effects produced per turn:** (not yet implemented)
...
- **Jobseekers** get visible postings.
...
- **Hiring managers** get forwarded candidates, interview records, and recruiter messages.
